  ...
  util.validateCredentials(answers.storeName, answers.apiKey, answers.apiPassword, function(err) {
    if (err) {
      ...
      if (args[3]) {
        ...
        repo = 'pixelcabin/ziplines_blank_template';
      }

      var gitClone = ['git', 'clone', 'https://github.com/' + repo + '.git', answers.storeName];
      ...
      exec(gitClone, function(err, out, code) {
        ...
        installers.push(function(callback) {
          ...
          exec(['grunt', 'compile:development'], function (err, out, code) {
            if (err) {
              console.log(err);
            } else {
